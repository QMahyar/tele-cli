...
pub async fn run_fanout(
    ...
    let parallel = flags.parallel.clamp(1, 3);
    ...
    while let Some(res) = tasks.join_next().await {
        match res {
            ...
            Err(join) => {
                return Err(TeleError::Other(format!("account task panicked: {join}")));
            }
        }
    }
    ...
    if !flags.quiet {
        for o in &outcomes {
            if let Some(err) = &o.error {
                log_line("error", &format!("{}: {err}", o.account));
            }
        }
    }
    Ok(crate::output::Envelope::new(outcomes))
}

pub fn select_accounts(flags: &GlobalFlags) -> TeleResult<Vec<String>> {
    if let Some(name) = &flags.account {
        return Ok(vec![name.clone()]);
    }
    let cfg = crate::config::load_config(flags.config_path.as_deref())?;
    let mut names: BTreeSet<String> = crate::session::list_session_names().into_iter().collect();
    if let Some(tag) = &flags.tag {
        ...
        names = names.intersection(&tagged).cloned().collect();
    }
