...
`tele msg get --chat X --id N --replied` additionally fetches the message that N replies to via grammers' `get_reply_to_message` (resolves `InputReplyTo` including cross-chat discussion parents for channel-post comments) and embeds it as an additive `replied_to` object on the fetched row. Requires a single `--id` (or a deep-link message id in `--chat`); rejected for `--ids` batches. When the message has no reply parent, the row simply carries no `replied_to` key. Serve/MCP: `GetParams.replied` (default false).
...
`tele msg export --account A --chat X [--format txt|jsonl] [--out FILE] [--limit N] [--offset-id M] [--since T] [--until T]` streams a chat history export through `iter_messages` and the shared row serializer. Default format is `jsonl` (one message JSON object per line); `txt` emits a human transcript (`#id [date] sender [media_kind]: text`). Rows carry an additive `link` field with the t.me permalink (`https://t.me/<username>/<id>` for public channels/supergroups, `https://t.me/c/<internal>/<id>` for private ones; absent for users and basic groups). Without `--out`, rows stream to stdout under the standard envelope (`data.messages`); with `--out FILE` the envelope instead carries `{chat, format, out, count, scanned, truncated}` and the file is created with private permissions (sensitive basenames refused). Export always requires an explicit single account (`--account`/`--tag`); `"truncated": true` means the limit was reached or the scan stopped early — more matching messages may exist. Without `--since`/`--until` the scan is server-capped at `limit + 1` messages; with date filters the scan stops after skipping `10×limit` (minimum 100) non-matching messages so a window that selects nothing cannot walk the entire history (the no-budget lane has no other deadline). Serve/MCP: `msg export` op (Read lane, no op timeout), `ExportParams` mirrors the flags with `deny_unknown_fields`.

## `msg scheduled`

`tele msg scheduled --account A --chat X [--limit N]` lists messages scheduled via `--schedule` (`messages.getScheduledHistory`). The payload carries `{chat, scheduled: [{id, date, message}]}`; `date` is an RFC 3339 string — the same machine date format as every other message row (`message_to_json`), not a Unix-epoch integer. Serve/MCP: `msg_scheduled` op (read_only), `ScheduledParams` mirrors the flags with `deny_unknown_fields`.
