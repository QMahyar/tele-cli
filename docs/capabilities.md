...
| kernel.session-port | Session export/import across machines + Telethon `.session` converter. Native export/import shipped (sha256 rows, locked-source refusal, --force semantics, restricted perms). Telethon converter: native-session AUTHORING from raw auth_key/dc works via public `Session` trait; READING Telethon SQLite blocked pending a SQLite-reader dependency approval (libsql preferred — already transitive) | `tele account export-session` / `import-session [--from-telethon]`, `src/session.rs` | want |
