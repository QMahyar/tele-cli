...
| msg.forward | Forward (silent, no notification) | `messages.forwardMessages` | raw `ForwardMessages` with `silent: true` (grammers 0.10 `forward_messages` hardcodes `silent: false`, so the friendly path is bypassed; new ids map back via `UpdateMessageId` and re-fetch) | `tele msg forward` | done |
...
| msg.effect | Animated effects: list available + apply on send | `/api/effects` | raw `messages.GetAvailableEffects` for list; raw `SendMessage`/`SendMedia` with `effect` for apply (grammers 0.10 builder carries no effect field) | `tele raw` messages.GetAvailableEffects; `tele msg send --effect ID` (text, single-file, and checklist sends; albums/--url/--copy-from/--as/--poll rejected; Premium/1-on-1 limits surface as honest server errors) | done |
| msg.checklist | Checklists: create + render, append items, toggle completion | `/api/todo` | raw `messages.{AppendTodoList,ToggleTodoCompleted}` + `SendMedia` with `inputMediaTodo` for create (grammers 0.10 `Media::from_raw` drops `ToDo`, so rows render from the raw `messageMediaToDo`) | `tele msg send --todo TITLE --todo-item T [--todo-item T ...] [--todo-others-can-append] [--todo-others-can-complete]`; `tele raw` registry arms for append/toggle; `tele msg get` / `search` rows carry additive `todo` object next to `poll` | done |
| msg.translate | Translation: friendly wrap over `messages.TranslateText` | `/api/translation`, `messages.TranslateText` | raw arm with result shaping | `tele msg translate (--chat X --ids N[,M] \| --text T [--text T ...]) --to-lang LANG` (peer+ids or free text; quota/Premium limits surface as honest server errors) | done |
| msg.transcribe | Voice transcription: friendly wrap + rating | `/api/transcribe`, `messages.TranscribeAudio`, `messages.RateTranscribedAudio` | raw arm with transcribed-audio shaping | `tele msg transcribe --chat X --id N` (weekly trial quota and group-boost unlocks surface as honest server errors); `tele msg transcribe-rate --chat X --id N --transcription-id T --good` (`--bad` rates negatively) | done |
...
| chat.forum | Forums / topics | `/api/forum` | raw | `tele topic *` incl. lifecycle close / reopen / edit / delete / pin (`topic create --emoji` accepts a single-codepoint emoji, resolved to a custom-emoji document id via `messages.searchCustomEmoji`, or a document id directly) | done |
...
Note on topic icons: `tele topic create --emoji` accepts a single-codepoint emoji (4 UTF-8 bytes) or a custom-emoji document id directly. Emoji input is resolved to a document id via `messages.searchCustomEmoji` before `messages.createForumTopic` runs, so the previous server-side degrade is gone; an emoji with no custom-emoji match fails with an honest invocation error. Empty values, non-emoji values, and multi-codepoint values fail with a Usage error before connect. Topic lifecycle (`close`, `reopen`, `edit`, `delete`, `pin`) ships over raw TL: `messages.editForumTopic` sets the closed flag and the title, `messages.updatePinnedForumTopic` pins, and `messages.deleteTopicHistory` removes whole-topic history (it is not `messages.deleteHistory`). `topic list` rows carry additive `closed` and `pinned`.
...
| mcp | MCP stdio server: tele mcp exposes 90 ops as tools via rmcp 3.1 (stdio, legacy handshake, inputSchema via schemars, annotations, confirm gate, read-only/groups filters) | `tele mcp --account NAME` | done | |
