...
| msg.poll | Polls: create, render in message rows + vote | `/api/poll`, `messages.sendVote` (no CLI path to close a poll; the layer carries `poll.closed`/`inputMediaPoll` for an editMessage-based close that is unimplemented) | `sendMedia` + `InputMediaPoll` for create; raw arm; `Media::Poll` answers | `tele msg send --poll Q --option A --option B`; `tele msg vote --chat X --id N --option 1[,2]`; additive `poll` object on msg get/search rows | done |
