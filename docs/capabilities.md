...
|---|---|---|---|
| stories.* | Stories | want (subset shipped: see `stories.*` done row; remaining gaps: `editStory`, `getStoriesByID`, `togglePinnedToTop`, `getStoryViewsList`, `getAllStories` feed, `searchPosts`, albums, stealth mode, reactions-on-stories) | Extra surface |
