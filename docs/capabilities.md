...
| kernel.device-id | Per-account device identity (`device_model`/`system_version`/`app_version`/`lang_code`) fed to the client builder | config `[accounts.<name>]` keys → `ConnectionParams` (all four wired at grammers 0.10; defaults neutral when unset) | done |
