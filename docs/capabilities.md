...
| auth.password-manage | Cloud password lifecycle: set/change/remove via local PH2 (pbkdf2-hmac-sha512 ×100k replicated from grammers-crypto 0.10) + SRP proofs; recovery-email confirm/resend/cancel with auto-prompt on EMAIL_UNCONFIRMED; status display; 7-day reset start/decline | `account.{GetPassword,UpdatePasswordSettings,ConfirmPasswordEmail,ResendPasswordEmail,CancelPasswordEmail,ResetPassword,DeclinePasswordReset}` + `grammers_crypto` SRP + local PH2 | `tele account password --set/--change/--remove/--confirm-email CODE/--resend-email/--cancel-email/--status/--reset-start/--decline-reset` (no-echo prompts; dry-run never prompts; secrets never in output) | done |
